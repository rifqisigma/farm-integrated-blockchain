...
func SendEmailValidateEmail(toEmail, token string) {
	...
	mailer.SetBody("text/html", fmt.Sprintf(`<a href="http://localhost:8080/auth/gmail/verification?email=%s&token=%s">Klik di sini untuk verifikasi</a>`, toEmail, token))
	dialer := gomail.NewDialer("smtp.gmail.com", 587, os.Getenv("EMAIL_SENDER"), os.Getenv("APP_PASSWORD"))

	fmt.Println("Verification Link:")
	fmt.Printf("http://localhost:8080/auth/gmail/verification?email=%s&token=%s\n", toEmail, token)
...
func SendEmailResetPassword(toEmail, token string) {
	...
	fmt.Println("Verification Link:")
	fmt.Printf("http://localhost:8080/auth/gmail/reset-password?email=%s&token=%s\n", toEmail, token)
	if err := dialer.DialAndSend(mailer); err != nil {
		fmt.Println("Error sending email:", err)
	}
}
